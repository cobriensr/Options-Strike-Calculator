...
history mean, including dates *later* than B's row → future, wrong-ticker
...
failure class as the 2026-06-09 lease incident, which was fixed *only* for the
renewal task.
...
baseline but the win flag is still `row["structure_correct"]` — whether *Claude's*
...
- [ ] **AUD-M1** `api/history.ts:147-171, 285-290` — partially-failed Schwab fetch
  ...
  forever. Log-only, no Sentry. *Fix:* gate the long-TTL write on all five
  symbols ok; Sentry breadcrumb on per-symbol failure.
- [ ] **AUD-M2** `api/events.ts:337-341, 527-531` — FRED HTTP errors degrade to
  ...
  CPI flag. *Fix:* don't cache (or short-TTL) when any source errored; Sentry
  warning on FRED non-OK.
- [ ] **AUD-M3** `api/_lib/embeddings.ts:238-255, 300` — Neon DATE-as-Date: raw
  `a.date` interpolated into the Claude prompt → emits
  `[Wed Jun 11 2026 00:00:00 GMT+0000 (Coordinated Universal Time)]` per analog
  row. Same in `findSimilarLessons` (`source_date`, ~:104-109). *Fix:*
  `TO_CHAR(..., 'YYYY-MM-DD')` like the sibling queries already do.
- [ ] **AUD-M4** `api/analyses.ts:32-36` — `spx`/`vix`/`vix1d` are DECIMAL → Neon
  returns strings; cast `as number` is a lie. Same bug class that corrupted
  `peak_ceiling_pct` for 5 weeks; currently latent. *Fix:* `::float8` in SQL or
  `Number()` in `parseRow`.
- [ ] **AUD-M5** `api/_lib/analyze-context.ts:192-229, 557-605` — sequential
  ...
  *Fix:* two `Promise.all` groups; pay the test-mock reorder.
- [ ] **AUD-M6** `api/positions.ts:133-141` — snapshot-id lookup outside the
  ...
  surfaces it as a misleading `'Failed to parse CSV'`). *Fix:* move inside the
  try / `safeDb(..., [])` so `snapshotId` degrades to null.

### Crons — `api/cron/`

- [ ] **AUD-M7** Per-row awaited INSERT/UPDATE loops despite `bulk-upsert.ts`
...
- [ ] **AUD-M10** `api/cron/curate-lessons.ts:471-482` — per-review DB write
  ...
  green even if every review fails. *Fix:* `Sentry.captureException` in the catch.
- [ ] **AUD-M11** `api/cron/fetch-zero-dte-flow.ts:90-100` — degenerate-snapshot
  ...
  degenerate feed = silent 0DTE flow gap. *Fix:* one `Sentry.captureMessage`.
- [ ] **AUD-M12** `api/cron/fetch-es-options-eod.ts:77-116` — QC counters
  (`with_oi`, `with_iv`) computed but never checked; only `total_rows === 0`
  alerts. Sidecar drift writing NULL-OI stubs sails through. *Fix:* gate on
  `with_oi === 0` too.
- [ ] **AUD-M13** DST-hardcoded overnight windows:
  ...
  Globex (7:30–8:30 CT) is excluded from H/L/C/VWAP, and auto-prefill also *runs*
...
- [ ] **AUD-M14** `src/hooks/useChainData.ts:64-67` — transient fetch failure
  ...
  cycle (120s after backoff). *Fix:* keep last-good data when
...
- [ ] **AUD-M16** `src/hooks/useMarketData.ts:285-316` — side effects inside the
  ...
  per failure, halving the backoff threshold. *Fix:* compute
...
- [ ] **AUD-M17** `src/hooks/useMarketData.ts:344, 386` — quotes-poll backoff can
  ...
  start for a visitor who authenticates in another tab. *Fix:* adopt
...
- [ ] **AUD-M19** `src/components/PreMarketInput.tsx:54-80` — scrubbing to a date
  ...
  to analyze. *Fix:* reset all fields + flags at the top of the date-change
  effect.
- [ ] **AUD-M20** `src/components/PanelPrefsModal/PanelPrefsModal.tsx:207-221,
  ...
  correct. *Fix:* focus-trap loop, native `<dialog>`, or `inert` on the shell.
...
- [ ] **AUD-M25** `sidecar/src/health.py:972` + `sidecar/src/archive_query.py:62-139`
  ...
  history. *Fix:* `threading.Semaphore(2)` around archive query execution
...
- [ ] **AUD-M26** `sidecar/src/options_router.py:358-369` — stat upsert failures
  ...
  SIDE-016 class) silently rots `futures_options_daily`. *Fix:*
...
- [ ] **AUD-M27** sidecar `db.py`/`databento_client.py` — per-record synchronous
  ...
  a Neon stall. *Fix:* route bars/stats through `BatchedWriter` (already exists,
  tables are upsert-idempotent).
- [ ] **AUD-M28** `uw-stream/src/connector.py:99-106, 155-159` — repeated *clean*
  ...
  provider sheds over-cap connections. *Fix:* alert from the clean-close branch;
...
- [ ] **AUD-M29** `uw-stream/src/handlers/interval_ba.py:454-471` — alert rows
  ...
  push notifications are gone and `_fired` blocks re-fire. *Fix:* re-prepend
...
- [ ] **AUD-M30** `vercel.json:2` — `ignoreCommand` diffs only `HEAD^ HEAD`: a
  ...
  change**. *Fix:* one-commit-at-a-time habit (current), or compare against
...
- [ ] **AUD-M32** `tsconfig.json:27` — `scripts/` (5.7MB of TS incl. prod-DB
  ...
  Combined with AUD-M31: zero static checks on prod-DB scripts. *Fix:* minimal
...
- [ ] **AUD-M34** `tsconfig.json:5` — one tsconfig serves browser + server:
  ...
  Functions). *Fix:* split app/api tsconfigs (standard Vite solution-style).
- [ ] **AUD-M35** ci.yml has no `timeout-minutes` on any job (hung Playwright
  ...
  `DATABASE_URL`. *Fix:* timeouts (20 app / 30 e2e); pin or lockfile-install.
...
- [ ] **AUD-M39** `ml/src/enrich_lottery_outcomes.py:111-123` — per-fire
  ...
  batch forever. *Fix:* derive `{date}-trades.parquet` from fire date; one
  connection; mark dead fires.
- [ ] **AUD-M40** Assorted (one line each — see agent report for detail):
  ...
  pandas 3.
- [ ] **AUD-M41** scripts/ misc mediums: `backfill-vol-surface.mjs:110-114` +
...
- `fetch-gexbot-fast.ts:357-368` / `fetch-gexbot-strikes.ts:130-141` — >10
  failures collapse to one Sentry *warning*; confirm alert rules page on it.
