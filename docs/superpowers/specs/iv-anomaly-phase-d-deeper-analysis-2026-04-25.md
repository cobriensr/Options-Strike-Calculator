...
said *what the trade looked like along the way* — was the eventual
winner down 80% first? Phase C told us "vol/OI 200×+ has 1.7% win
rate." But it bucketed the *ratio*, never the absolute volume that
...
trade alerts in isolation — alerts are a *strike-selection filter*
...
- n alerts
- win rate using *per-(ticker, regime) re-picked best strategy* (NOT
...
Phase C bucketed signal *count*. We never bucketed signal *strength*.
...
Phase C never asked about *day-level* homogeneity or *firing-sequence*
position.
...
- Path-shape requires re-reading `strike_iv_snapshots` per alert —
  ...
  are NULL not zero.
- Composite intensity score is *only* meaningful when a signal fired;
...
- **Drawdown-aware exit strategies** (trailing stop, scale-out) —
  this study identifies that exits *should* be path-aware; designing
