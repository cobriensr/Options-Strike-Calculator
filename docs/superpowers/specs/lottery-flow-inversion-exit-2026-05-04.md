...
#### Task 1.2 — Adapt `exit_simulation.py` to write results back to DB

- [ ] New script `scripts/backfill-flow-inversion-exit.mjs` (Node — same idiom as `backfill-lottery-fires.mjs`) that:
  - Reads `ml/experiments/lottery-net-flow-eda/exit_simulation_results.parquet`
  - For each fire_id, UPDATE lottery_finder_fires SET realized_flow_inversion_pct = inversion_pct WHERE id = fire_id
  - Batched UPDATEs via UNNEST pattern from `scripts/backfill-day-vix-bucket.mjs`
- [ ] Run the existing `exit_simulation.py` to refresh the parquet, then run `backfill-flow-inversion-exit.mjs`
- **Verify:** `SELECT COUNT(*) FROM lottery_finder_fires WHERE realized_flow_inversion_pct IS NOT NULL` matches the parquet row count (~47,658)
...
2. **Cost-net vs gross display.** The simulation report shows two columns (`inversion_pct` gross and `inversion_net_pct` after costs). The DB column will store gross (matches existing realized_* convention). User can still mentally subtract their estimated frictions in the UI. Confirm this is the right call.
