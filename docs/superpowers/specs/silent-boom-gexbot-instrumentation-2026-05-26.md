...
- Add the 8 gex\_ columns to the INSERT statement.
...
- `api/__tests__/detect-silent-boom.test.ts` — extend the existing test
  to mock `getLatestGexbotSnapshot` and assert the gex\_ columns flow
...
- Add the 8 gex\_ columns to the SELECT list.
...
- Include the gex\_\* columns in the SELECT.
...
- **Live-feature wiring still deferred** — `LotteryAlertRow` /
  ...
  gex\_\* columns yet. The bundle would need a retrain that knows the
...
2. Manual probe in `docs/tmp/` confirms that fresh Silent Boom inserts
   today populate the gex\_\* columns end-to-end.
