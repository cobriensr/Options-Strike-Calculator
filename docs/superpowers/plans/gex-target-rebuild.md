...
7. **`gex_strike_0dte` historical data is permanently incomplete.** The UW `/stock/SPX/spot-exposures/expiry-strike` and `/spot-exposures/strike` endpoints both return only **the most recent snapshot per session** when queried with a historical `?date=` parameter — typically 2 timestamps per day (an early-day cache and a late-day cache). The `/spot-exposures` aggregate endpoint does return per-minute historical data, but it's aggregated across all strikes, not broken down per-strike, so it can't feed the scoring math. **Consequence**: the live `fetch-gex-0dte` cron is the only way to populate `gex_strike_0dte` with per-minute per-strike snapshots, and any historical day that wasn't captured live is permanently lost. Phase 4's `gex_target_features` backfill is correct but only writes feature rows for days that have intraday data in `gex_strike_0dte` — at the time of the Phase 4 rollout (2026-04-09), that was 2026-04-07 (partial), 2026-04-08 (full), and 2026-04-09 (live, in progress). The number of usable backtest days grows by 1 per trading day going forward as the cron captures each new session. Phase 7 must surface this gracefully via the data-availability UX (item 8 below), and Phase 10's tuning will need to wait until the table has built up ≥30 days of real cron data (~2026-05-09 minimum for a usable backtest window).

8. **Phase 7 must include data-availability UX in the scrubber** (Phase 4 follow-up). Because of item 7, the scrubber's date picker MUST query `gex_target_features` for available dates and only allow selecting dates that exist. Show a banner or tooltip explaining that historical data accumulates from the cron deploy date forward and that 30-day backfill from UW is not possible. The component should render a "no data for selected date" empty state cleanly rather than displaying empty charts. Implementation note: a single `SELECT DISTINCT date FROM gex_target_features ORDER BY date DESC` query at component mount gives the scrubber its valid date set; the cost is one extra query at first render and ~30-60 minutes of additional Phase 7 work. This is required for Phase 7 to ship a coherent UX.
