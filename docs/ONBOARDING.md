...
It is also a **research platform**: ~80 Vercel cron jobs (86 schedule entries in `vercel.json`) persist live market data to Neon Postgres; a multi-phase ML pipeline runs nightly to validate signals and build features; the analyze endpoint's "lessons learned" compendium curates each session's review into rules that get injected into the next.
...
| `VIXRangeAnalysis`    | Historical range + survival heatmap                                 | `src/data/vixRangeStats.ts` (static)                                |
...
| `DealerRegimeTile`    | Dealer net-gamma posture                                            | `/api/dealer-regime` (Unusual Whales)                               |
...
| `Periscope`         | UW Periscope screenshot reader (auto-playbook)            | `/api/periscope-*` (the `periscope-scraper` service was retired 2026-05-26) |
...
| `Tracker`         | Long-term contract tracker with alerts                    | `/api/tracker/*` (shipped — see `docs/superpowers/specs/contract-tracker-2026-05-17.md`) |
...
| Panel             | Shows                                                                                                         | Source                                          |
| ----------------- | ------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| `ChartAnalysis`   | Upload UI for 2 chart screenshots (Periscope Gamma, Periscope Charm) → structure / delta / entry plan / hedge | Anthropic Claude Opus 4.7 via `/api/analyze`    |
...
- **What it powers**: All flow signals (Market Tide, Net Flow, Greek Flow), GEX snapshots, dark pool blocks, websocket feeds (flow-alerts, option_trades for ~86 tickers), Periscope screenshots — basically half the data surface.
...
- **What it powers**: All persistent data — snapshots, analyses, flow_data, GEX tables, training_features, lessons, positions. ~85 tables, 190 migrations.
...
- **Required tier**: **Pro ($20/month)** — required for the 780s function timeout on `/api/analyze`. Hobby tier caps at 60s and the Claude Opus 4.7 with adaptive thinking will time out.
...
### Required for the futures sidecar (Railway service 1 of 3)

#### 10. Railway

- **What it powers**: Hosts the `sidecar/`, `uw-stream/`, and `classifier/` Python services.
- **Sign up**: [railway.app](https://railway.app).
- **Cost**: ~$5–20/month/service for personal usage tier. Three services = budget **~$15–60/month**.
...
### Required for the websocket consumer (Railway service 2 of 3)
...
| Railway (sidecar + uw-stream + classifier) | $15–60                      |
| OpenAI embeddings                          | <$5                         |
...
- [sidecar/README.md](../sidecar/README.md), [uw-stream/README.md](../uw-stream/README.md), [classifier/README.md](../classifier/README.md), [ml/README.md](../ml/README.md) — Python services
