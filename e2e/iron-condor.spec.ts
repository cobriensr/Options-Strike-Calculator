...
test.describe('Iron Condor Section', () => {
  ...
  test('hedge calculator shows delta selector when open', async ({ page }) => {
    ...
    // Multiple delta options as aria-pressed toggle buttons
    const deltaToggles = results.locator('button[aria-pressed]');
    expect(await deltaToggles.count()).toBeGreaterThanOrEqual(6);
  });
