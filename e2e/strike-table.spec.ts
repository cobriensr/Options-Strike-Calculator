...
test.describe('Delta Strikes Table', () => {
  ...
  test('put strikes are below spot and call strikes are above', async ({
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Column 1 = put strike (raw), column 7 = call strike (raw)
      const putText = await cells.nth(1).textContent();
      const callText = await cells.nth(7).textContent();
  ...
  test('higher delta produces narrower strikes', async ({ page }) => {
    ...
    const widePut = Number.parseFloat((await firstCells.nth(1).textContent())!);
    const narrowPut = Number.parseFloat(
      (await lastCells.nth(1).textContent())!,
    );
