...
**Method:** for each alert, finds the top-3 abs*gex strikes for that (date, expiry) and checks the position of the \_nearest* one relative to current spot. `alert_in_gex_zone` is true when the alert strike is between spot and the nearest top-3 GEX.
