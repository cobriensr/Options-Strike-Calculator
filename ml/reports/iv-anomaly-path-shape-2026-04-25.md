...
**Caveat:** premium trajectory truncates when the alert's strike crosses ITM (snapshot table only stores OTM strikes). MAE*to_close is therefore the worst drawdown \_on the OTM portion* of the path — does NOT capture post-ITM drawdowns (which exist when an ITM premium retraces and the strike re-OTMs).

## Drawdown before peak — eventual winners vs losers

Reading: a winner with median MAE -40% means the median _eventual winner_ went down 40% before bouncing.
...
Big numbers means "eventual winners endured deep drawdowns first". Indicates the regime/ticker is _psychologically punishing_ even when right.
