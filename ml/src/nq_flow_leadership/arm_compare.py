...
def main() -> int:
    ...
    overall["arm"] = overall["ticker"].map(
        lambda t: (
            "NDX-complex"
            if t in NDX_COMPLEX
            else ("SPX-complex" if t in SPX_COMPLEX else "other")
        )
    )
    ...
    # Per-arm aggregate.
    per_arm = []
    for arm_name, members in [
        ("NDX-complex", NDX_COMPLEX),
        ("SPX-complex", SPX_COMPLEX),
    ]:
        ...
        per_arm.append(
            {
                ...
                "mean_abs_rho_among_bonf_sig": (
                    ...
                    if (sub["p_bonf"] < 0.05).any()
                    else None
                ),
                "max_abs_rho": round(float(sub["abs_rho"].max()), 4),
                "top10_mean_abs_rho": round(
                    float(sub.nlargest(10, "abs_rho")["abs_rho"].mean()), 4
                ),
            }
        )
    ...
    for feat in overall["feature"].unique():
        ...
        ndx_max = (
            sub[sub["arm"] == "NDX-complex"]["abs_rho"].max()
            if (sub["arm"] == "NDX-complex").any()
            else 0
        )
        spx_max = (
            sub[sub["arm"] == "SPX-complex"]["abs_rho"].max()
            if (sub["arm"] == "SPX-complex").any()
            else 0
        )
        per_feature.append(
            {
                ...
                "winner": "SPX-complex"
                if spx_max > ndx_max
                else ("NDX-complex" if ndx_max > spx_max else "tie"),
                "gap": round(float(abs(spx_max - ndx_max)), 4),
            }
        )
    per_feature.sort(
        key=lambda x: max(x["ndx_complex_max_abs_rho"], x["spx_complex_max_abs_rho"]),
        reverse=True,
    )
    ...
    if spx_arm["top10_mean_abs_rho"] > ndx_arm["top10_mean_abs_rho"] * 1.15:
        ...
        verdict = (
            "NDX-complex meaningfully outperforms — stay on NQ, use NDX-complex flow"
        )
    else:
    ...
    for f in per_feature:
        print(
            f"  {f['feature']:<14}: NDX-max={f['ndx_complex_max_abs_rho']:.3f}  "
