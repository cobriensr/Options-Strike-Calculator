...
def compute_nq_session_features(nq: pd.DataFrame) -> pd.DataFrame:
    ...
    nq['cum_pv'] = grouped.apply(lambda g: (typical.loc[g.index] * g['volume']).cumsum())
    ...
    nq['ret_30m_back'] = log_close - grouped['close'].apply(lambda s: np.log(s).shift(30))
...
def compute_signal_thresholds(feats: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, d in enumerate(unique_dates):
        if i < WARMUP_DAYS:
            ...
            continue
        trailing_dates = unique_dates[i - TRAILING_DAYS : i]  # last N days, excluding today
...
def summarize(trades: list[Trade]) -> dict:
    ...
    return {
        ...
        'avg_win_pct': round(float(wins['net_return_pct'].mean()), 4) if len(wins) else None,
        'avg_loss_pct': round(float(losses['net_return_pct'].mean()), 4) if len(losses) else None,
        ...
        'profit_factor': round(
            ...
            if len(losses) and losses['net_return_pct'].sum() != 0 else float('nan'),
            3,
        ),
...
def main() -> int:
    ...
    print(f'Walk-forward: {WARMUP_DAYS}-day warmup, trailing-{TRAILING_DAYS}-day percentile')
    print(f'Entry: signal in bottom {ENTRY_PERCENTILE}%, time in open|pm, NQ>VWAP, no flush')
    print(f'Exit: TP +{TP_PCT*100:.2f}% / SL -{SL_PCT*100:.2f}% / time {HOLD_MAX_MIN}m')
    ...
    if summary.get('n_trades', 0) > 0:
        ...
        print()
        print('--- TRADE LOG ---')
        print(trades_df[['entry_ts', 'bucket', 'signal_value', 'entry_price',
                         'exit_price', 'exit_reason', 'hold_minutes', 'net_return_pct']].to_string(index=False))
