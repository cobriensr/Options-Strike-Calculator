...
def load_one_day(path: Path) -> pd.DataFrame:
    ...
    df["is_sweep"] = (
        df["report_flags"].fillna("").str.contains("intermarket_sweep", regex=False)
    )
...
def reindex_to_full_grid(
    prims: pd.DataFrame, day_minute_grid: pd.DatetimeIndex
) -> pd.DataFrame:
...
def apply_rolling_and_derive(prims: pd.DataFrame, expiry_label: str) -> pd.DataFrame:
    ...
    primitive_cols = [
        "total_premium",
        "total_trade_count",
        "signed_delta_premium",
        "signed_otm_vega_premium",
        "signed_otm_delta_premium",
        "ask_premium",
        "bid_premium",
        "sweep_count",
        "sweep_premium",
        ...
        "put_ask_premium",
        "put_bid_premium",
        "call_ask_premium_otm",
        "call_bid_premium_otm",
        "put_ask_premium_otm",
        "put_bid_premium_otm",
    ]
    ...
    for ticker in PREDICTOR_TICKERS:
        sub = (
            prims[prims["underlying_symbol"] == ticker]
        ...
        for win in ROLLING_WINDOWS:
            ...
            ask_plus_bid = (rolled["ask_premium"] + rolled["bid_premium"]).replace(
                0, np.nan
            )
            call_plus_put = (
                rolled["call_ask_premium"] + rolled["put_ask_premium"]
            ).replace(0, np.nan)
            sweep_total_count = (
                rolled["bull_sweep_count"] + rolled["bear_sweep_count"]
            ).replace(0, np.nan)
            sweep_total_prem = (
                rolled["bull_sweep_premium"] + rolled["bear_sweep_premium"]
            ).replace(0, np.nan)
            otm_premium_total = (
                rolled["call_ask_premium_otm"]
                + rolled["call_bid_premium_otm"]
                + rolled["put_ask_premium_otm"]
                + rolled["put_bid_premium_otm"]
            ).replace(0, np.nan)
            ...
            features = {
                ...
                "ncp": (rolled["call_ask_premium"] - rolled["call_bid_premium"])
                / total,
                ...
                / otm_premium_total,
                # ------- Original
                "call_put_imb": (rolled["call_ask_premium"] - rolled["put_ask_premium"])
                / call_plus_put,
            }
...
def main() -> int:
    ...
    p.add_argument(
        "--limit", type=int, default=None, help="Process only N days (timing probe)"
    )
    args = p.parse_args()
    ...
    for path in files:
        ...
        print(
            f"{date_part}: {len(feats):>4} minute rows, {len(feats.columns):>4} cols, {t1 - t0:.1f}s"
        )
    ...
    if args.limit is None:
        ...
        for c in sample_cols:
            ...
            print(
                f"  {c}: {nn}/{len(final)} non-null, range=[{final[c].min():.3g}, {final[c].max():.3g}]"
            )
