...
def assign_bucket(minute_of_day_ct: pd.Series) -> pd.Series:
    ...
    bucket = pd.Series(['outside'] * len(minute_of_day_ct), index=minute_of_day_ct.index)
...
def main() -> int:
    ...
    for _, row in top.iterrows():
        ...
        if not peers.empty:
            ...
            concentration = float(peers['abs_rho'].max() / peers['abs_rho'].mean()) if peers['abs_rho'].mean() > 0 else float('nan')
            diag.append({
                'ticker': row['ticker'],
                'feature': row['feature'],
    ...
    for d in diag[:15]:
        sig = '***' if d['overall_p_bonf'] < 0.05 else (' **' if d['overall_p'] < 0.01 else '  *')
        print(
    ...
    print('          * = uncorrected p<0.05 only (weak; could be multiple-comparison artifact)')
