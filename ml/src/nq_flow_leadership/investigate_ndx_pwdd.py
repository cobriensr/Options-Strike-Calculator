...
def main() -> int:
    ...
    for d, sub in pm.groupby('date_ct'):
        ...
        print(f'  {d}: n={n:>3}  rho={rho:+.3f}  p={p:.3f}' if not np.isnan(rho) else f'  {d}: n={n:>3}  insufficient data')
    ...
    if valid_days:
        ...
        print(f'  -> Days with neg rho: {n_neg}/{len(valid_days)},  pos: {n_pos}/{len(valid_days)},  median: {median_rho:+.3f}')
    ...
    for name, (lo, hi) in sub_buckets.items():
        ...
        rho, p, n = spearman_safe(sub[target], sub[ret_col])
        print(f'  {name}: n={n:>4}  rho={rho:+.3f}  p={p:.2e}' if not np.isnan(rho) else f'  {name}: n={n:>4}  insufficient')
    ...
    print(f'  date         time(CT)   NDX_pwdd_30m  fwd_ret_60m')
    for _, r in top10.iterrows():
        h, m = divmod(int(r['hour_min_ct']), 60)
        sign_match = ((r[target] < 0) and (r[ret_col] > 0)) or ((r[target] > 0) and (r[ret_col] < 0))
        marker = '   <-- contrarian fired' if sign_match else ''
        print(f'  {r["date_ct"]}  {h:02d}:{m:02d}      {r[target]:+.3f}        {r[ret_col]:+.5f}{marker}')

    # Distribution check: how many of the top |target| events fired contrarian?
    contrarian_hits = ((top10[target] < 0) & (top10[ret_col] > 0)) | ((top10[target] > 0) & (top10[ret_col] < 0))
    print(f'  -> Contrarian fired: {int(contrarian_hits.sum())}/{len(top10)} of top-10 extreme PM PWDD minutes')
    ...
    out = {
        ...
        'q2_per_day': per_day,
        'q3_sub_localization': {
            name: dict(zip(['rho','p','n'], spearman_safe(
                merged[(merged['hour_min_ct'] >= lo) & (merged['hour_min_ct'] < hi)][target],
                merged[(merged['hour_min_ct'] >= lo) & (merged['hour_min_ct'] < hi)][ret_col],
            )))
            for name, (lo, hi) in sub_buckets.items()
        },
        'q4_peers': {
        ...
        'q6_top10_drivers': top10.assign(
            date_ct=top10['date_ct'].astype(str)
