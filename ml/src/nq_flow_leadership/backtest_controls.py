...
from backtest import (  # type: ignore
    SIGNAL_COL, ENTRY_PERCENTILE, TP_PCT, SL_PCT, HOLD_MAX_MIN,
    TRAILING_DAYS, WARMUP_DAYS, RT_COST_PCT, FLUSH_THRESHOLD,
    OPEN_MIN, OPEN_END, PM_MIN, PM_END,
    Trade, load_data, compute_nq_session_features, compute_signal_thresholds,
    time_bucket, summarize,
)
    ...
    *,
    ...
    if use_signal:
        if invert_signal:
            ...
            mask &= merged[SIGNAL_COL].notna() & merged['threshold_top20'].notna() \
                & (merged[SIGNAL_COL] >= merged['threshold_top20'])
        else:
            mask &= merged[SIGNAL_COL].notna() & merged['threshold'].notna() \
    ...
    for entry_idx in eligible_idx:
        ...
        for j in range(entry_idx + 1, end_idx + 1):
            ...
            if bar['date_ct'] != entry_date:
                ...
                break
            if bar['low'] <= sl_level:
                exit_idx = j; exit_price = sl_level; exit_reason = 'sl'; break
            if bar['high'] >= tp_level:
                exit_idx = j; exit_price = tp_level; exit_reason = 'tp'; break
        ...
        sig_val = float(entry_row[SIGNAL_COL]) if pd.notna(entry_row[SIGNAL_COL]) else float('nan')
        trades.append(Trade(
...
def add_top20_threshold(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, d in enumerate(unique_dates):
        ...
        threshold_by_date[d] = float(np.percentile(vals, 100 - ENTRY_PERCENTILE)) if len(vals) >= 100 else float('nan')
...
def main() -> int:
    ...
    variants = {
        'ORIGINAL: bottom-20% + filters': {
            'use_signal': True, 'invert_signal': False,
            'use_time_filter': True, 'use_vwap_filter': True, 'use_flush_filter': True,
        },
        'A: INVERTED top-20% + filters': {
            'use_signal': True, 'invert_signal': True,
            'use_time_filter': True, 'use_vwap_filter': True, 'use_flush_filter': True,
        },
        'B: Pure signal, NO filters': {
            'use_signal': True, 'invert_signal': False,
            'use_time_filter': False, 'use_vwap_filter': False, 'use_flush_filter': False,
        },
    ...
    for name, kwargs in variants.items():
        ...
        print(name)
        print('=' * 70)
        for k in ('n_trades', 'win_rate_pct', 'avg_win_pct', 'avg_loss_pct',
                  'expectancy_pct', 'net_total_return_pct', 'max_drawdown_pct',
                  'profit_factor', 'exit_reason_breakdown', 'bucket_breakdown',
                  'avg_hold_minutes'):
            v = summary.get(k)
    ...
    rand_trades = simulate_variant(
        signals, nq_aug,
        use_signal=False, use_time_filter=True, use_vwap_filter=True, use_flush_filter=True,
        random_n=n_orig,
    )
    rand_summary = summarize(rand_trades)
    results['D: RANDOM matched-count'] = rand_summary
    for k in ('n_trades', 'win_rate_pct', 'avg_win_pct', 'avg_loss_pct',
              'expectancy_pct', 'net_total_return_pct', 'avg_hold_minutes',
