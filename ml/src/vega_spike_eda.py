...
analytical questions:
  ...
  1. Distribution comparison vs control — Mann-Whitney U
  2. Directionality hit-rate — Binomial test + Wilson CIs
  3. Time-to-peak — fwd_5m / fwd_15m / fwd_30m trace plot
  4. Magnitude effect — z_score vs |fwd_return_15m| Theil-Sen regression
  5. Time-of-day stratification — AM / midday / PM boxplots
  6. Confluence vs solo comparison
...
def compute_fwd_returns(
    ...
    """
    Compute fwd_return_Nm for each spike event using etf_candles_1m.

    Strategy: for each spike, find the candle bar whose timestamp equals
    the spike timestamp (the spike bar's close), then look up the candle
    at (timestamp + N minutes). Return = (close_at_N - close_at_0) / close_at_0.

    If the spike DB column is already non-null, use it; otherwise compute.
    """
    # Index candles by (ticker, timestamp) for O(1) lookup
    ...
    for _, spike in spikes.iterrows():
        ...
        # Find the close price at the spike bar — use next candle at or after t0
        # (minute bars: the bar labelled t has close at end of that minute)
        base_close = candle_index.get((ticker, t0))
        if base_close is None:
            # Try t0 truncated to minute
        ...
        for h in horizons_min:
            ...
            # Use DB value if already populated
            existing = spike.get(col)
...
def build_control_samples(
    ...
    Forward returns are then computed for each control bar at 15-minute horizon.
    """
    ...
    for _, spike in spikes.iterrows():
        ...
        # Exclude spike neighbourhood (capture exclusions as default arg to avoid
        # late-binding closure over loop variable)
        ...
        sampled = candidates.sample(n=n_draw, random_state=int(rng.integers(1e9)))
        ...
        for _, cand in sampled.iterrows():
            ...
            fwd_close = candle_index.get((ticker, t15))
            if fwd_close is None:
                continue
            control_rows.append(
                {
                    "ticker": ticker,
                    "fwd_return_15m": (fwd_close - base_close) / base_close,
...
def plot_distribution_comparison(
    ...
    """Overlaid histograms + KDE of fwd_return_15m, spike vs control."""
    spike_vals = spikes["fwd_return_15m"].dropna().values
    ctrl_vals = control["fwd_return_15m"].dropna().values

    if len(spike_vals) == 0:
        print("  WARNING: No spike fwd_return_15m values — skipping distribution plot")
        return {}
    if len(ctrl_vals) == 0:
        print("  WARNING: No control fwd_return_15m values — skipping distribution plot")
        return {}
    ...
    return {
        ...
        "spike_median_15m": float(np.median(spike_vals)),
        "control_median_15m": float(np.median(ctrl_vals)),
        "mw_u": float(u_stat),
        "mw_p": float(p_mw),
    }
...
def plot_directionality(spikes: pd.DataFrame, out_path: Path) -> dict:
    """Hit-rate of sign(fwd_return_15m) == sign(dir_vega_flow), with Wilson CIs."""
    df = spikes[spikes["fwd_return_15m"].notna() & spikes["dir_vega_flow"].notna()].copy()
    if len(df) == 0:
        print("  WARNING: No valid rows for directionality plot — skipping")
        return {}

    df["sign_flow"] = np.sign(df["dir_vega_flow"])
    df["sign_ret"] = np.sign(df["fwd_return_15m"])
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Directionality: sign(fwd_return_15m) == sign(dir_vega_flow)\n95% Wilson CIs, Binomial test vs 50% null")
...
def plot_time_to_peak(spikes: pd.DataFrame, out_path: Path) -> dict:
    """All spike traces [fwd_5m, fwd_15m, fwd_30m] + median ribbon, faceted by sign."""
    horizons = [5, 15, 30]
    cols = [f"fwd_return_{h}m" for h in horizons]

    df = spikes.copy()
    df["spike_sign"] = np.sign(df["dir_vega_flow"])
    df_pos = df[df["spike_sign"] > 0].dropna(subset=cols, how="all")
    df_neg = df[df["spike_sign"] < 0].dropna(subset=cols, how="all")
    ...
    for ax, subset, sign_label, base_color in [
        ...
        for _, row in subset.iterrows():
            ys = [row.get(c, np.nan) for c in cols]
            if any(pd.notna(y) for y in ys):
                ax.plot(xs, ys, color=base_color, alpha=0.2, lw=1)

        # Median + IQR ribbon
        medians = [float(subset[c].median()) for c in cols]
        q25s = [float(subset[c].quantile(0.25)) for c in cols]
        q75s = [float(subset[c].quantile(0.75)) for c in cols]

        ax.plot(xs, medians, color=base_color, lw=2.5, zorder=5, label="Median")
        ax.fill_between(xs, q25s, q75s, color=base_color, alpha=0.25, label="IQR")
    ...
    fig.suptitle("Time-to-peak: Forward returns at 5m / 15m / 30m horizons", y=1.01)
    plt.tight_layout()
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {out_path.name}")
...
def plot_magnitude_scatter(spikes: pd.DataFrame, out_path: Path) -> dict:
    """z_score vs |fwd_return_15m| scatter + Theil-Sen regression."""
    df = spikes[spikes["fwd_return_15m"].notna() & spikes["z_score"].notna()].copy()
    if len(df) < 4:
        print("  WARNING: Not enough data for magnitude scatter — skipping")
        return {}

    df["abs_ret_15m"] = df["fwd_return_15m"].abs()
    ...
    ax.set_title("z_score vs |fwd_return_15m| — Theil-Sen regression")
    ax.set_xlabel("Z-score (spike magnitude)")
    ax.set_ylabel("|15-min Forward Return|")
...
def plot_tod_stratification(spikes: pd.DataFrame, out_path: Path) -> dict:
    """Boxplots of fwd_return_15m by AM/midday/PM, separated by spike sign."""
    df = spikes[spikes["fwd_return_15m"].notna()].copy()
    if len(df) == 0:
    ...
    for ax, sign_label, color in [
        ...
        for period in periods:
            pdata = subset[subset["session_period"] == period]["fwd_return_15m"].dropna()
            if len(pdata) == 0:
    ...
    fig.suptitle("fwd_return_15m by time-of-day period (9:30-11:30 / 11:30-13:30 / 13:30-close ET)", y=1.01)
    plt.tight_layout()
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {out_path.name}")
...
def plot_confluence_vs_solo(spikes: pd.DataFrame, out_path: Path) -> dict:
    """Comparison of fwd_return_15m for confluence vs solo spikes."""
    df = spikes[spikes["fwd_return_15m"].notna()].copy()
    if len(df) == 0:
    ...
    xlabels = []
    if n_conf > 0:
        data_groups.append(conf_df["fwd_return_15m"].values)
        xlabels.append(f"Confluence\n(n={n_conf})")
    if n_solo > 0:
        data_groups.append(solo_df["fwd_return_15m"].values)
    ...
    ax1.set_title(f"fwd_return_15m: Confluence vs Solo\n(WARNING: n_confluence={n_conf} — eyeball only)")
    ax1.set_ylabel("15-min Forward Return")
    ...
    ax2 = axes[1]
    if n_solo > 0:
        ax2.scatter(solo_df["z_score"], solo_df["fwd_return_15m"].abs(),
                    color="#5bc8f5", alpha=0.7, s=50, label=f"Solo (n={n_solo})", zorder=3)
    if n_conf > 0:
        ax2.scatter(conf_df["z_score"], conf_df["fwd_return_15m"].abs(),
                    color="#ffcc00", alpha=0.9, s=120, marker="*",
                    label=f"Confluence (n={n_conf})", zorder=5)
    ax2.set_title("z_score vs |fwd_return_15m| by confluence")
    ax2.set_xlabel("Z-score")
    ax2.set_ylabel("|15-min Forward Return|")
    ...
    if n_conf > 0:
        stats_out["confluence_median_15m"] = round(float(conf_df["fwd_return_15m"].median()), 5)
        stats_out["confluence_mean_15m"] = round(float(conf_df["fwd_return_15m"].mean()), 5)
    if n_solo > 0:
        stats_out["solo_median_15m"] = round(float(solo_df["fwd_return_15m"].median()), 5)
        stats_out["solo_mean_15m"] = round(float(solo_df["fwd_return_15m"].mean()), 5)
    return stats_out
...
def write_findings_md(
    ...
    lines = [
        ...
        "",
        f"**Sample**: {n_total} total spike events; {n_with_fwd} with computable forward returns "
        f"(fwd_return columns were NULL in DB — computed here from etf_candles_1m).",
        "",
    ...
    if dist_stats:
        ...
        p_str = f"{p_mw:.4f}" if p_mw >= 0.0001 else "<0.0001"
        spike_med = dist_stats.get("spike_median_15m", float("nan"))
        ctrl_med = dist_stats.get("control_median_15m", float("nan"))
        lines += [
            f"Spike fwd_return_15m median: **{fmt_pct(spike_med)}**, "
            ...
            "A p-value below 0.05 would indicate the spike return distribution is meaningfully "
    ...
    if dir_stats:
        ...
        if ov:
            ...
            lines += [
                ...
                "Hit rate > 50% means spikes correctly predict the 15-min price direction; "
    ...
    if peak_stats:
        for sign_key in ["positive", "negative"]:
            ...
            if ps:
                lines.append(
                    f"**{sign_key.capitalize()} spikes** (n={ps.get('n', 0)}): "
        ...
        lines += [
            "",
            "If returns compound monotonically (5m < 15m < 30m in absolute terms), "
            "the spike effect persists beyond 15 minutes. If 15m > 30m, there is mean reversion.",
            "",
        ]
    else:
        lines += ["Insufficient data to compute.", ""]
    ...
    if mag_stats:
        ...
        lines += [
            ...
            "A positive slope means larger z-scores are associated with larger absolute forward returns; "
    ...
    if tod_stats:
        ...
        lines += [
            "",
            "AM spikes (before 11:30 ET) may have more price impact due to lower liquidity and "
            "wider bid-ask spreads. PM spikes near close may be affected by MOC order flow.",
            "",
        ]
    else:
        lines += ["Insufficient data to compute.", ""]
    ...
    if conf_stats:
        ...
        c_med = conf_stats.get("confluence_median_15m")
        s_med = conf_stats.get("solo_median_15m")
        lines += [
            f"Confluence events: n={n_c}. Solo events: n={n_s}. "
            + (f"Confluence median 15m return: {fmt_pct(c_med)}. " if c_med is not None else "")
            + (f"Solo median 15m return: {fmt_pct(s_med)}. " if s_med is not None else "")
    ...
    lines += [
        "## Caveats",
        "",
        f"- Total spike events: {n_total}. Events with computable fwd returns: {n_with_fwd}. "
        "Some spikes may lack fwd returns because they occurred after the last candle ingested "
        "(etf_candles_1m goes to 2026-04-24; spikes on 2026-04-27 are excluded).",
        "- The QQQ spike on 2026-03-17 predates candle coverage (candles start 2026-03-18) "
        "and produces NaN — correctly excluded.",
        "- Spikes near the end of the session (after ~15:45 ET) lack 30-min forward return candles; "
        "they still contribute to 5m and 15m analyses where candles are available.",
...
def main() -> None:
    ...
    n_with_fwd = int(spikes["fwd_return_15m"].notna().sum())
    print(f"  Spikes with fwd_return_15m: {n_with_fwd}/{len(spikes)}")

    if n_with_fwd == 0:
        print("\nERROR: No spikes have computable 15m forward returns. Check candle coverage.")
        sys.exit(1)

    spikes_valid = spikes[spikes["fwd_return_15m"].notna()].copy()
    ...
    # 3. Time-to-peak (use all spikes that have at least one horizon)
    ...
    if dir_stats.get("Overall"):
        ...
        print(
            f"  Hit rate: {ov['k']}/{ov['n']} = {ov['rate'] * 100:.1f}%, "
