...
def load_data() -> pl.DataFrame:
    ...
    df = df.with_columns(
        ...
        (
            (
                pl.col("expiry").cast(pl.Datetime).dt.replace_time_zone("UTC")
                - pl.col("first_ts")
            ).dt.total_days()
        ).alias("dte"),
        (pl.col("strike") / pl.col("first_underlying").fill_null(1.0) - 1).alias(
            "moneyness"
        ),
    )
    return df
...
def apply_checklist(df: pl.DataFrame, thresholds: dict) -> pl.DataFrame:
    ...
    for tkr, thresh in thresholds.items():
        threshold_expr = threshold_expr | (
            (pl.col("ticker") == tkr) & (pl.col("total_premium") >= thresh)
        )
    qual = df.filter(
        threshold_expr
        ...
        & (
            (
                ...
                & (pl.col("moneyness").abs() <= 0.05)
            )
            | pl.col("first_underlying").is_null()
        )
    ).with_columns(
        pl.when(pl.col("ask_pct") >= 0.85)
        .then(pl.lit("ASK"))
        .otherwise(pl.lit("BID"))
        .alias("side"),
    )
    qual = qual.with_columns(
        pl.when((pl.col("side") == "BID") & (pl.col("option_type") == "put"))
        .then(pl.lit("bullish"))
        .when((pl.col("side") == "BID") & (pl.col("option_type") == "call"))
        .then(pl.lit("bearish"))
        .when((pl.col("side") == "ASK") & (pl.col("option_type") == "put"))
        .then(pl.lit("bearish"))
        .otherwise(pl.lit("bullish"))
        .alias("direction"),
    )
    return qual
...
def detect_pairs(qual: pl.DataFrame, all_df: pl.DataFrame) -> dict:
    ...
    for r in qual.iter_rows(named=True):
        ...
        ovl = (
            min(r["last_ts"], m["last_ts"]) - max(r["first_ts"], m["first_ts"])
        ).total_seconds()
...
def plot_01_daily_footprint(qual: pl.DataFrame, output_dir: Path) -> None:
    for ticker in ["SPXW", "NDXP"]:
        ...
        fig.suptitle(
            f"Daily Whale Footprint — {ticker}", fontsize=14, fontweight="bold"
        )
...
def plot_02_timeline(qual: pl.DataFrame, df: pl.DataFrame, output_dir: Path) -> None:
    ...
    for ax, ticker in zip(axes, ["SPXW", "NDXP"]):
        ...
        if price_df.height > 0:
            ...
            ax.plot(
                xs, ys, color="#404040", linewidth=1.5, alpha=0.6, label="Median spot"
            )
        ...
        ax.set_title(
            f"{ticker} — 11-day whale timeline", fontsize=12, fontweight="bold"
        )
        ax.set_ylabel("Strike")
        ax.set_xlabel("Date")
        ax.tick_params(axis="x", rotation=45)
    fig.suptitle(
        "Whale Strike Levels vs. Underlying — 11 Days", fontsize=14, fontweight="bold"
    )
...
def plot_03_todays_level_map(
    qual: pl.DataFrame, df: pl.DataFrame, output_dir: Path
) -> None:
    ...
    spot_df = (
        ...
        .select(["first_ts", "first_underlying"])
        .sort("first_ts")
    )
    if spot_df.height > 0:
        times = [
            t.replace(tzinfo=None)
            for t in spot_df["first_ts"]
            .dt.convert_time_zone("America/Chicago")
            .to_list()
        ]
        prices = spot_df["first_underlying"].to_list()
        ax.plot(
            times, prices, color="#404040", linewidth=1.5, alpha=0.7, label="SPX spot"
        )
    for r in today.iter_rows(named=True):
        ...
        ax.text(
            0.99,
            r["strike"],
            f"  {r['ticker']} {r['strike']:.0f}{r['option_type'][0].upper()} "
...
def plot_04_outcome_scatter(qual: pl.DataFrame, output_dir: Path) -> None:
    ...
    qual2 = (
        qual.filter(pl.col("first_underlying").is_not_null())
        .with_columns(
            pl.col("first_ct").dt.hour().alias("hour"),
        )
        .with_columns(
            pl.when(
                (pl.col("direction") == "bullish")
                & (pl.col("last_underlying") >= pl.col("strike"))
            )
            .then(pl.lit("hit"))
            .when(
                (pl.col("direction") == "bearish")
                & (pl.col("last_underlying") <= pl.col("strike"))
            )
            .then(pl.lit("hit"))
            .otherwise(pl.lit("miss"))
            .alias("outcome"),
            ((pl.col("strike") / pl.col("first_underlying") - 1) * 100).alias(
                "pct_to_strike"
            ),
        )
    )
    ...
    for ax, direction in zip(axes, ["bullish", "bearish"]):
        ...
        for r in sub.iter_rows(named=True):
            ...
            ax.scatter(
                ...
                marker=marker,
                alpha=0.7,
                edgecolors="black" if r["outcome"] == "hit" else None,
            )
        ax.axhline(0, color="black", linewidth=0.5)
        n_hit = sub.filter(pl.col("outcome") == "hit").height
        ax.set_title(
            f"{direction.capitalize()} whales: {n_hit}/{sub.height} hit "
            f"({n_hit / sub.height * 100:.0f}%)",
            fontsize=12,
        )
        ax.set_xlabel("Hour CT (print time)")
        ax.set_ylabel("% from spot to target strike")
    fig.suptitle(
        "Whale Outcome Scatter — did price reach the strike?",
        fontsize=14,
        fontweight="bold",
    )
...
def plot_05_time_to_target(qual: pl.DataFrame, output_dir: Path) -> None:
    qual2 = qual.with_columns(
        ((pl.col("last_ts") - pl.col("first_ts")).dt.total_minutes()).alias(
            "duration_min"
        ),
        pl.col("first_ct").dt.hour().alias("hour"),
    )
    grid = (
        qual2.group_by(["ticker", "hour"])
        .agg(
    ...
    for i, _tkr in enumerate(tickers):
        for j, _hr in enumerate(hours):
            v = matrix[i, j]
            if not np.isnan(v):
                ax.text(
                    j,
    ...
    ax.set_title(
        "Median Whale Chain Duration — proxy for time-to-target",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_06_rr_distribution(qual: pl.DataFrame, output_dir: Path) -> None:
    ...
    qual2 = qual.filter(pl.col("first_underlying").is_not_null()).with_columns(
        ((pl.col("strike") / pl.col("first_underlying") - 1).abs() / STOP_PCT).alias(
            "rr"
        ),
    )
    ...
    bins = np.linspace(
        0, max(np.max(bull), np.max(bear)) if len(bull) and len(bear) else 10, 20
    )
    ax.hist(
        bull,
        bins=bins,
    ...
    ax.set_ylabel("Count")
    ax.set_title(
        "Whale R:R Distribution — strike-to-spot vs assumed stop",
        fontsize=12,
        fontweight="bold",
    )
    ax.legend()
...
def plot_07_premium_distribution(
    df: pl.DataFrame, thresholds: dict, output_dir: Path
) -> None:
    ...
    for j in range(len(INDEX_ETF), len(axes)):
        axes[j].set_visible(False)
    fig.suptitle(
        "Per-Ticker Premium Distribution (11-day outsized universe)",
        fontsize=14,
        fontweight="bold",
    )
...
def plot_08_filter_funnel(df: pl.DataFrame, thresholds: dict, output_dir: Path) -> None:
    ...
    stages = [
        ...
        (
            "After per-ticker p95",
            df.filter(pl.col("ticker").is_in(INDEX_ETF) & threshold_expr).height,
        ),
        (
            "After trade_count ≥5",
            df.filter(
...
def plot_09_trade_count_distribution(
    df: pl.DataFrame, thresholds: dict, output_dir: Path
) -> None:
    threshold_expr = pl.lit(False)
    for tkr, thresh in thresholds.items():
    ...
    sub = df.filter(
        pl.col("ticker").is_in(INDEX_ETF) & threshold_expr & (pl.col("dte") <= 14)
    )
    fig, ax = plt.subplots(figsize=(10, 6), constrained_layout=True)
    ...
    ax.set_title(
        "Trade-Count Distribution — premium-qualified chains",
        fontsize=12,
        fontweight="bold",
    )
    ax.legend()
...
def plot_10_alignment_matrix(qual: pl.DataFrame, output_dir: Path) -> None:
    ...
    for r in grid.iter_rows(named=True):
        if r["ticker"] in tickers and r["trade_date"] in dates:
            matrix[tickers.index(r["ticker"]), dates.index(r["trade_date"])] = r[
                "net_score"
            ]
    ...
    ax.set_title(
        "Cross-Ticker Whale Alignment — net direction by day",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_11_synthetic_detector(
    qual: pl.DataFrame, pairs: dict, output_dir: Path
) -> None:
    ...
    if alone_pts:
        ax.scatter(
            *zip(*[(p[0], p[1]) for p in alone_pts]),
            s=[p[2] for p in alone_pts],
            c="steelblue",
            alpha=0.6,
            edgecolors="black",
            label=f"Alone (n={len(alone_pts)})",
        )
    if seq_pts:
        ax.scatter(
            *zip(*[(p[0], p[1]) for p in seq_pts]),
    ...
    ax.set_ylabel("Strike")
    ax.set_title(
        "Pairing Status of Whale Prints — synthetic vs roll vs standalone",
        fontsize=12,
        fontweight="bold",
    )
    ax.legend()
...
def plot_12_minute_clustering(qual: pl.DataFrame, output_dir: Path) -> None:
    qual2 = qual.with_columns(
        (pl.col("first_ct").dt.hour() * 60 + pl.col("first_ct").dt.minute()).alias(
            "min_of_day"
        ),
    )
    ...
    ax.set_title(
        "Whale Print Clustering by Minute of Day — 11 days combined",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_13_strike_magnet(qual: pl.DataFrame, output_dir: Path) -> None:
    ...
    for ax, ticker in zip(axes, ["SPXW", "NDXP"]):
        ...
        ax.imshow(matrix, cmap="RdYlGn", aspect="auto", vmin=-1, vmax=1)
        ax.set_xticks(range(len(dates)))
        ax.set_xticklabels(dates, rotation=45, ha="right", fontsize=8)
        ax.set_yticks(range(len(strikes)))
        ax.set_yticklabels([f"{s:.0f}" for s in strikes], fontsize=8)
        ax.set_title(
            f"{ticker} — Whale strike levels by day", fontsize=11, fontweight="bold"
        )
        ax.set_xlabel("Date")
        ax.set_ylabel("Strike")
    fig.suptitle(
        "Strike-Level Magnet Score — green=bullish whale, red=bearish whale",
        fontsize=13,
        fontweight="bold",
    )
...
def main() -> int:
    ...
    plot_calls = [
        ...
        (
            "07_premium_distribution",
            lambda: plot_07_premium_distribution(df, thresholds, OUTPUT_DIR),
        ),
        ("08_filter_funnel", lambda: plot_08_filter_funnel(df, thresholds, OUTPUT_DIR)),
        (
            "09_trade_count_distribution",
            lambda: plot_09_trade_count_distribution(df, thresholds, OUTPUT_DIR),
        ),
        ("10_alignment_matrix", lambda: plot_10_alignment_matrix(qual, OUTPUT_DIR)),
        (
            "11_synthetic_detector",
            lambda: plot_11_synthetic_detector(qual, pairs, OUTPUT_DIR),
        ),
...
def print_pipeline_summary(latest_date) -> None:
    ...
    print(
        "  • Cumulative:     scripts/eod-flow-analysis/output/cumulative-headlines.txt"
    )
    print(f"  • Vercel Blob:    flow/year={y}/month={m}/day={d}/data.parquet")
    print(f"  • Archive:        ~/Desktop/Bot-Eod-parquet/{iso}-trades.parquet")
    print("  • Plots:          ml/plots/whale-detection/*.png")
    print()
    print(
        "Next: review cumulative-headlines.txt, scan plots, commit per-day artifacts."
    )
