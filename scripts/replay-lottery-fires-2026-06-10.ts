 ...
 * (8:30 → ~10:25 CT), scanning the same 7-min window the cron uses, and
 * inserts qualifying fires with ON CONFLICT DO NOTHING.
 *
 * The window covers the outage gap (8:30 → uw-stream resume) and
 * overlaps the live 10:08+ CT fires; ON CONFLICT (option_chain_id,
 * trigger_time_ct) DO NOTHING dedups that overlap.
 *
 ...
 *   npx tsx scripts/replay-lottery-fires-2026-06-10.ts            # dry-run
 *   npx tsx scripts/replay-lottery-fires-2026-06-10.ts --apply    # writes
 */
