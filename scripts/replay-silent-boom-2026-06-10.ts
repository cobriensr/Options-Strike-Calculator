 ...
 * 2026-06-10. Walks minute-by-minute through 13:30 UTC → 15:25 UTC
 * (8:30 → ~10:25 CT), scanning the same 35-min trailing window the cron
 * uses, and inserts qualifying fires with ON CONFLICT DO NOTHING.
 *
 * Window covers the uw-stream outage gap and overlaps the live 10:08+
 * CT fires; ON CONFLICT (option_chain_id, bucket_ct) DO NOTHING dedups
 * that overlap.
 *
 ...
 *
 * Structural template: scripts/replay-lottery-fires-2026-05-20.ts.
 ...
 *   npx tsx scripts/replay-silent-boom-2026-06-10.ts            # dry-run
 *   npx tsx scripts/replay-silent-boom-2026-06-10.ts --apply    # writes
 */
...
const REPLAY_DAY_ET = '2026-06-10';
const REPLAY_START_UTC = new Date('2026-06-10T13:30:00Z');
const REPLAY_END_UTC = new Date('2026-06-10T15:25:00Z');
const STEP_MS = 60_000;
// --apply writes to silent_boom_alerts; default is dry-run (counts only).
...
async function processOneCronTick(
  ...
  for (const { g, f, dte } of allFires) {
    ...
    if (!APPLY) {
      // Dry-run: count as if inserted, no DB write.
      inserted += 1;
      continue;
    }
