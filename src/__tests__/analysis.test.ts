...
describe('buildPreviousRecommendation', () => {
  it('includes structure, delta, confidence, and reasoning for a basic result', () => {
    ...
    expect(output).toContain('Reasoning: Low VIX environment with range-bound price action');
  });
  ...
  it('includes all fields when fully populated', () => {
    ...
    expect(output).toContain('Stop conditions: VIX spikes above 20; SPX breaches short strike');
  });
