 ...
 *
 * Schedule: every 5 minutes during market hours (in vercel.json)
 *
  ...
  try {
    ...
    if (rows.length > 0) {
      const largest = rows.reduce((best, r) => {
        const a = Math.abs(Number.parseFloat(r.call_gex)) +
          Math.abs(Number.parseFloat(r.put_gex));
        const b = Math.abs(Number.parseFloat(best.call_gex)) +
