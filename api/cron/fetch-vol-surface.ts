...
async function fetchRealizedVol(
  apiKey: string,
): Promise<RealizedVolRow[]> {
  return uwFetch<RealizedVolRow>(
    apiKey,
    '/stock/SPX/volatility/realized',
  );
}
...
async function storeRealizedVol(
  ...
  const iv30d =
    Number.parseFloat(String(latestRv.implied_volatility)) ||
    null;
  const rv30d =
    Number.parseFloat(String(latestRv.realized_volatility)) ||
    null;

  // Compute derived metrics
  const ivRvSpread =
    iv30d !== null && rv30d !== null ? iv30d - rv30d : null;
...
export default async function handler(
  ...
  try {
    ...
    const rvStored = await storeRealizedVol(
      rvRows,
      ivRankRows,
      today,
    );
