...
export async function engineerPhase2Features(
  ...
  if (prevDayRows.length > 0) {
    ...
    if (features.vix != null && prev.vix_close != null) {
      features.prev_day_vix_change =
        features.vix - Number(prev.vix_close);
    }
  }
  ...
  if (prices.length >= 6) {
    ...
    if (logReturns5.length >= 5) {
      ...
      features.realized_vol_5d =
        Math.sqrt(variance5) * Math.sqrt(252) * 100;
    }
  }
  ...
  if (
    ...
    features.vix_term_slope =
      (features.vix9d - features.vix1d) / features.vix;
  }
  ...
  if (features.vvix != null) {
    ...
    if (vvixHistory.length >= 10) {
      ...
      const belowCount = vvixValues.filter(
        (v) => v <= features.vvix!,
      ).length;
  ...
  if (eventRows.length > 0) {
    ...
    const types = new Set(
      eventRows.map((r) => r.event_type as string),
    );
    const priority = [
      'FOMC',
      'CPI',
  ...
  `;
  if (
    nextEventRow.length > 0 &&
    nextEventRow[0]!.next_date != null
  ) {
    ...
    if (
      ...
      features.days_to_next_event = Math.round(
        (nextDate.getTime() - thisDate.getTime()) /
          (24 * 60 * 60 * 1000),
      );
    }
  }
  ...
  try {
    ...
    logger.warn(
      { err: mpErr },
      'Max pain feature extraction failed',
    );
  }
  ...
  try {
    ...
    logger.warn(
      { err: dpErr },
      'Dark pool feature extraction failed',
    );
  }
  ...
  try {
    ...
    if (apiKey) {
      ...
      if (ovRes.ok) {
        ...
        if (Array.isArray(ovData) && ovData.length > 0) {
          ...
          const callVol = Number.parseInt(
            String(ov.call_volume ?? '0'),
            10,
          );
          const putVol = Number.parseInt(
            String(ov.put_volume ?? '0'),
            10,
          );
          ...
          const callPrem = Number.parseFloat(
            String(ov.call_premium ?? '0'),
          );
          const putPrem = Number.parseFloat(
            String(ov.put_premium ?? '0'),
          );
          ...
          if (bullPrem + bearPrem > 0) {
            features.opt_premium_ratio =
              bullPrem / (bullPrem + bearPrem);
          }
    ...
    logger.warn(
      { err: ovErr },
      'Options volume feature extraction failed',
    );
  }
}
