...
export async function savePeriscopeAnalysis(
  ...
  try {
    const rows = await sql`
      ...
      ON CONFLICT (trading_date, slot_captured_at, auto_generated) DO NOTHING
      RETURNING id
    `;
    ...
    // ON CONFLICT DO NOTHING fired — another request already inserted the
    // row for this slot. Look it up so the caller still gets a usable id
    // instead of a null (which the auto-playbook endpoint treats as a
    // save failure and 500s on). Only meaningful for auto-generated rows
    // since slot_captured_at is null for manual reads.
    if (!autoGenerated || slotCapturedAt == null) return null;
    const existing = await sql`
      ...
      LIMIT 1
    `;
    const existingRaw = existing[0]?.id;
