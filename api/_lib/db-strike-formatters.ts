/**
 * Pure formatters for per-strike Greek exposure data.
 *
 * Everything in this file takes typed rows (StrikeExposureRow, FlowDataRow,
 * NetGexRow) or domain objects (ZeroGammaAnalysis) and produces a
 * Claude-readable string block. No DB access, no network — the query
 * functions in `db-strike-helpers.ts` call these after loading the raw
 * data.
 *
 * Mirrors the existing `db-flow.ts` / `analyze-context-formatters.ts`
 * pattern: query functions stay in `db-strike-helpers.ts`, formatters
 * live here. Output is bit-identical to the previous in-place formatters.
 *
 * Phase 5f of docs/superpowers/specs/api-refactor-2026-05-02.md.
 */
