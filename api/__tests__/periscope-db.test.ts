...
describe('savePeriscopeAnalysis', () => {
  ...
  // ON CONFLICT recovery path: when two auto-playbook requests race
  // the unique (trading_date, slot_captured_at, auto_generated) index,
  // the loser's INSERT returns no rows. savePeriscopeAnalysis follows
  // up with a SELECT to return the existing row's id so the caller
  // still gets a usable id instead of treating it as a save failure.
  it('recovers existing id when auto-generated INSERT hits ON CONFLICT', async () => {
    // 1st call: INSERT returns [] (conflict fired)
    mockSql.mockResolvedValueOnce([]);
    // 2nd call: SELECT returns the existing row's id
    mockSql.mockResolvedValueOnce([{ id: 42 }]);
    const id = await savePeriscopeAnalysis(
      saveInput({
        autoGenerated: true,
        slotCapturedAt: '2026-05-19T15:30:00.000Z',
      }),
    );
    expect(id).toBe(42);
  ...
  it('does NOT run the recovery SELECT for manual rows (slot_captured_at = null)', async () => {
    // Manual rows have slot_captured_at = null; the unique index
    // never fires on NULL, so an empty RETURNING means a genuine
    // failure and should bubble up as null without an extra SELECT.
    mockSql.mockResolvedValueOnce([]);
    const id = await savePeriscopeAnalysis(saveInput()); // autoGenerated default false
    expect(id).toBeNull();
    expect(mockSql).toHaveBeenCalledTimes(1);
  });
