...
describe('E2E: 2026-04-23 informed-flow flush regression', () => {
  ...
  it('does not flag any ticker other than SPY/QQQ (flow hid in ETF channel per spec)', () => {
    ...
    // tickers (SPXW / NDXP / IWM from the 2026-04-24 rescope, plus
    // NVDA / SNDK from the single-name expansion) perfectly flat AND
    // at a sub-5× vol/OI ratio, so any flag from them would indicate
    // either an accidental signal leak (noise generator, stddev floor
    // regression) or a vol/OI-gate bypass.
